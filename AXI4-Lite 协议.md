...
这个 DUT 是一个 AXI-to-SPI Bridge，它内部所有寄存器的功能都围绕着 SPI 工作参数展开。CPU 原生支持 AXI 协议，不需要额外的协议翻译就能发起总线事务，但 AXI 只负责运输数据，不负责理解数据的语义。寄存器层承担了语义转换的角色：AXI 接口将 CPU 发来的地址和数据正确接收后，寄存器层将其翻译成 SPI master 需要的具体配置（如 CPOL/CPHA 模式、时钟分频、帧间隙时序等）。这些寄存器在行业术语中称为 CSR（Control and Status Register，控制状态寄存器），而非传统的 register file，因为每个寄存器有独立的地址和功能——有的控制 SPI 启动（slv_reg0），有的查询忙状态（slv_reg1），有的发送数据（slv_reg8），有的接收数据（slv_reg9）。寄存器层还提供了频率适配的缓冲作用：CPU 在 GHz 级别完成写入后可以立刻处理其他事务，SPI 在 MHz 级别逐 bit 串行移位由 SPI master 独立完成。

...
AXI4-Lite 按字节地址寻址。这个 DUT 的地址总线宽度是 6bit，最多可以寻址 64 字节（$2^6$）。数据总线宽度是 32bit（4 字节），因此每次传输读取或写入 4 字节。为了在字节地址和 32bit 数据之间对齐，地址的最低 2bit 固定为 0，从第 3bit 开始作为寄存器选择。RTL 中通过两个参数实现这个映射：ADDR_LSB 设为 2，表示跳过最低的 2 个字节地址位；OPT_MEM_ADDR_BITS 设为 3，表示用 3bit 来选择 8 个 32bit 寄存器。
